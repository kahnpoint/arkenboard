...
iKeyr := new iKey("r", "_", "4", "{F4}")
iKeyt := new iKey("g", "/*{Space}{Space}*/{Left 3}", "5", "{F5}")

iKeyy := new iKey("y", "", "6", "{F6}")
iKeyu := new iKey("u", "_", "7", "{F7}")
iKeyi := new iKey("i", "{Up}","8", "{F8}")
iKeyo := new iKey("o", "`=", "9", "{F9}")
iKeyp := new iKey("{Esc}", "", "0", "{F10}")

iKeya := new iKey("w", "`/", "#", "{F11}")
iKeys := new iKey("a", "*", "@", "{F12}")
iKeyd := new iKey("e", "{Backspace}","{Backspace}", "{RWin Down}.{RWin Up}")
iKeyf := new iKey(" ",  "{Delete}", "{Delete}", "")
iKeyg := new iKey("t", "{Tab}", "{Tab}", "")

iKeyh := new iKey("h", "φ", "τ", "π") ;"θ", "Δ", "λ")
